...
class ControllerModuleSobfeedback extends Controller {
	private $error = array();

	public function index(){
		$this->load->language('module/sobfeedback');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if ($this->validateForm($this->request->post)) {
				...
				if (!isset($this->request->get['module_id'])) {
					...
					$this->response->redirect($this->url->link('module/sobfeedback', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL'));
				}
			}
		}
		...
		$fedbacks = $this->model_module_sobfeedback->getFeedbackAll();
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('module/sobfeedback', 'token=' . $this->session->data['token'], 'SSL')
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('module/sobfeedback', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('module/sobfeedback', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('module/sobfeedback', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
	
		$data['feedback_list'] = $this->url->link('module/sobfeedback/getlist', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->response->setOutput($this->load->view('module/sobfeedback.tpl', $data));
	...
	public function getList() {
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('module/sobfeedback/getlist', 'token=' . $this->session->data['token'], 'SSL')
		);

		$data['action'] = $this->url->link('module/sobfeedback', 'token=' . $this->session->data['token'], 'SSL');

		$data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');
		$data['add_module'] = $this->url->link('module/sobfeedback', 'token=' . $this->session->data['token'], 'SSL');

		$data['deleteFeedback'] = $this->url->link('module/sobfeedback/deleteFeedback', 'token=' . $this->session->data['token'], 'SSL');
		$data['saveFeedback'] = $this->url->link('module/sobfeedback/saveFeedback', 'token=' . $this->session->data['token'], 'SSL');
		...
		$feedbacks = $this->model_module_sobfeedback->getFeedbackAll($filter_data);
		$feedback_total = $this->model_module_sobfeedback->getTotalFeedbacks($filter_data);

		
		$module_options = $this->model_module_sobfeedback->getFeedbackOptions('');
		$data['module_names'] = $this->model_module_sobfeedback->getFeedbackOptions('module_name');
		...
		foreach ($module_options as $module_option) {
			$data['feedback_module_options'][] = array(
				...
				'module_url'  => $this->url->link('module/sobfeedback&module_id='.$module_option['module_id'], 'token=' . $this->session->data['token'], 'SSL'),
				);
			}
		...
		$data['sort_name'] = $this->url->link('module/sobfeedback/getlist', 'token=' . $this->session->data['token'] . '&sort=module_name' . $url, 'SSL');
		$data['sort_date'] = $this->url->link('module/sobfeedback/getlist', 'token=' . $this->session->data['token'] . '&sort=date' . $url, 'SSL');
		$data['sort_id'] = $this->url->link('module/sobfeedback/getlist', 'token=' . $this->session->data['token'] . '&sort=fedback_id' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('module/sobfeedback/getlist', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('module/sobfeedback_list.tpl', $data));
	...
	public function deleteFeedback(){
		...
		if ($this->request->post['selected']){
			...
			foreach ($this->request->post['selected'] as $feedback_id) {
				...
				$this->model_module_sobfeedback->deleteFeedback($feedback_id);
			}
		
			$this->response->redirect($this->url->link('module/sobfeedback/getlist', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	public function saveFeedback(){
		...
		if ($this->request->post){
				...
				$this->model_module_sobfeedback->saveFeedback($this->request->post);
				...
				if (isset($feedback_forms['sobfeedback_field'])){
					...
					foreach ($feedback_forms as $feedback_form) {
						...
						if (isset($feedback_form['option'])) {
							if (utf8_strlen($feedback_form['option'][$language_id]) < 1 || utf8_strlen($feedback_form['option'][$language_id]) > 100) {
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/sobfeedback')) {
	...
	public function install(){
		...
		$this->model_module_sobfeedback->install();
	}
