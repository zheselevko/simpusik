...
class ControllerCommonHeader extends Controller {
	public function index() {
		...
		if (!isset($this->request->get['token']) || !isset($this->session->data['token']) || ($this->request->get['token'] != $this->session->data['token'])) {
            ...
            // modification
    		$data['refresh'] = $this->url->link('extension/modification/refresh', 'token=' . $this->session->data['token'], 'SSL');
    		$data['clear'] = $this->url->link('extension/modification/clear', 'token=' . $this->session->data['token'], 'SSL');
			...
			$data['affiliate_approval'] = $this->url->link('marketing/affiliate', 'token=' . $this->session->data['token'] . '&filter_approved=1', 'SSL');
				...
				// Feedbacks 
					...
					/*echo "<pre>";
					var_dump($feedback_total = $this->model_module_sobfeedback->getTotalFeedbacks(array('filter_noread' => 'noread')));
					echo "</pre>";*/

					if ($this->model_module_sobfeedback->getTotalFeedbacks() !== false) {
						$feedback_total = $this->model_module_sobfeedback->getTotalFeedbacks(array('filter_noread' => 'noread'));
						
						$data['feedback_total'] = $feedback_total;
						$data['feedback_noread'] = $this->url->link('module/sobfeedback/getlist', 'token=' . $this->session->data['token'] . '&filter_noread=noread', 'SSL');
