...
class ControllerProductCategory extends Controller {
	public function index() {
		...
		if ($category_info) {
            ...
            if ($page > 1) {
                $str_page = ' Страница ' . num_propis($page);
                $this->document->setRobots('noindex, follow');
            } else {
                $str_page = '';
            }

			$this->document->setTitle($category_info['meta_title'] . $str_page);
			$this->document->setDescription($category_info['meta_description']);
			$this->document->setKeywords($category_info['meta_keyword']);
			$this->document->addLink($this->url->link($route, $path), 'canonical');
			$this->document->addLink($this->url->link($route, $path), 'canonical');
			...
			$data['heading_title']  = $category_info['name'] . $str_page;
			$data['seo_h1']         = $category_info['seo_h1'] . $str_page;
    		...
    		$results_hit = $this->model_catalog_product->getProducts($filter_data_hit);

    		if ($results_hit) {
    			foreach ($results_hit as $result) {
    				$data['products_hit'][] = $this->getProduct($result, $setting);
    			}
    		}
            ...
            /* Next */
            if ($page < $num_pages) {
                $this->document->addLink($this->url->link($route, $path . '&page=' . ($page + 1)), 'next');
            }

            /* Prev */
            if ($page > 1) {
                if ($page == 2) {
                    $this->document->addLink($this->url->link($route, $path), 'prev');
                } else {
                    $this->document->addLink($this->url->link($route, $path . '&page=' . ($page - 1)), 'prev');
                }
            }
