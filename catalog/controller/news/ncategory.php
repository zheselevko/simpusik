...
class ControllerNewsNcategory extends Controller {  
	public function index() {
        ...
        $data['news_total'] = $this->model_catalog_news->getTotalNews();
	...
	protected function getPageContent($settings) {
		...
		$data['display_style'] = $this->config->get('ncategory_bnews_display_style');
            ...
            $num_pages = ceil($news_total / $limit);

            /* Next */
            if ($page < $num_pages) {
                $this->document->addLink($this->url->link('news/ncategory', 'path=' . $this->request->get['path'] . '&page=' . ($page + 1)), 'next');
            }

            /* Prev */
            if ($page > 1) {
                if ($page == 2) {
                    $this->document->addLink($this->url->link('news/ncategory', 'path=' . $this->request->get['path']), 'prev');
                } else {
                    $this->document->addLink($this->url->link('news/ncategory', 'path=' . $this->request->get['path'] . '&page=' . ($page - 1)), 'prev');
                }
            }
