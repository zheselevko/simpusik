	...
	public function add() {
		...
		if ($product_info) {
			...
			if (!$json) {
        		...
        		$json['text_items'] = sprintf($this->language->get('text_items'), $cart_count, num2word($cart_count), $this->currency->format($total));
	...
	public function addKitToCart() {
        ...
        if (!$json) {
    		...
    		if (!empty($this->request->post['products'])) {
                $parameters = explode(',', $this->request->post['products']);

    			$json['products'] = $this->request->post['products'];
        ...
        if ($total_qty) {
			$json['success'] = sprintf($this->language->get('text_success_kit'), $this->url->link('product/product', 'product_id=' . $this->request->post['product_id']), $product_info['name'], $total_qty, $this->url->link('checkout/cart'));
