...
class ControllerCommonQuicksignup extends Controller {
	...
	public function login() {
		...
		if ($this->customer->isLogged()) {
			...
			if (!$this->customer->login($this->request->post['account_auth_name'], $this->request->post['account_auth_password'])) {
				$json['error'] = $this->language->get('error_login');
			}
			$customer_info = $this->model_account_customer->getCustomerByEmail($this->request->post['account_auth_name']);
	...
	public function login2() {
        ...
        //$user['email'] - соц. сеть, через которую авторизовался пользователь
        ...
        $this->model_account_customer->getCustomerByEmail($user['email']);
