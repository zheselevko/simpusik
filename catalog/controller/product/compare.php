...
class ControllerProductCompare extends Controller {
	public function index() {
		...
		foreach ($this->session->data['compare'] as $key => $product_id) {
			...
			if ($product_info) {
                if ($product_info['image'] && file_exists(DIR_IMAGE . $product_info['image'])) {
					...
					$image = $this->model_tool_image->resize('placeholder.png', $this->config->get('config_image_compare_width'), $this->config->get('config_image_compare_height'));
				}
				...
				$attribute_data[0]['text']  = $product_info['location'];
                $attribute_values[0][]      = $product_info['location'];

				foreach ($attribute_groups as $attribute_group) {
					foreach ($attribute_group['attribute'] as $attribute) {
                        $attribute_data[$attribute['attribute_id']]['different'] = '';
						$attribute_data[$attribute['attribute_id']]['text'] = $attribute['text'];

                        $attribute_values[$attribute['attribute_id']][] = $attribute['text'];
					}
				}
				...
				$data['attributes'][0]['name']      = 'Коллекция';
				$data['attributes'][0]['different'] = '';

				foreach ($attribute_groups as $attribute_group) {
					foreach ($attribute_group['attribute'] as $attribute) {
						$data['attributes'][$attribute['attribute_id']]['name'] = $attribute['name'];
						$data['attributes'][$attribute['attribute_id']]['different'] = '';
					}
				}
			} else {
				unset($this->session->data['compare'][$key]);
		...
		foreach ($data['attributes'] as $key => $attribute) {
            if (count($attribute_values[$key])) {
                $tmp_value = $attribute_values[$key][0];
                foreach($attribute_values[$key] as $value) {

                    if ($tmp_value != $value) {
            		    $data['attributes'][$key]['different'] = 'different';
                    }
                    $tmp_value = $value;
                }
            }
		...
		foreach ($data['products'] as $key => $product) {
            foreach ($data['products'][$key]['attribute'] as $attr_key => $attribute) {
    		    $data['products'][$key]['attribute'][$attr_key]['different'] = $data['attributes'][$attr_key]['different'];
            }
        }
