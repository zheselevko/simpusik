...
class ModelCatalogProduct extends Model {
	...
	public function getSitemapProducts() {
		$product_data = $this->cache->get('product.sitemap.' . (int)$this->config->get('config_language_id'));

		if (!$product_data) {
			$query = $this->db->query("SELECT p.product_id, p.date_added, p.date_modified FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY p.date_added DESC");

			$product_data = $query->rows;

			$this->cache->set('product.sitemap.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id'), $product_data);
		}
	...
	public function getProductRating($product_id) {
        $query = $this->db->query("SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r WHERE r.product_id = '" . $product_id . "' AND r.status = '1' GROUP BY r.product_id");
