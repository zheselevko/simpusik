...
class ModelCatalogNcomments extends Model {		
	public function addComment($news_id, $data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "sb_ncomments SET author = '" . $this->db->escape($data['name']) . "', reply_id = '" . (int)$data['reply_id'] . "', news_id = '" . (int)$news_id . "', text = '" . $this->db->escape(strip_tags($data['text'])) . "', language_id = '" . (int)$this->config->get('config_language_id') . "', date_added = NOW()");
	}
