...
function validator(){
    ...
    if ( $('.contacts-leave-message form').length ) {
        ...
        form_item.validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    url: 'index.php?route=module/feedback/send2',
                    type: 'post',
                    dataType: 'json',
                    data: $("#form-contacts-message").serialize(),
                    beforeSend: function() {
                    },
                    complete: function() {
                    },
        ...
        form_item2.validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    ...
                    type: 'post',
                    dataType: 'json',
                    data: $("#form-account-login").serialize(),
                    beforeSend: function() {
                    },
                    complete: function() {
                    },
                ...
                $.ajax({
                    ...
                    success: function(json) {
                        ...
                        if (json['success']) {
                            ...
                            $('input[name=\'leave_review_name\']').val('');
                            $('input[name=\'leave_review_email\']').val('');
                            $('textarea[name=\'leave_review_text\']').val('');
                            $('input[name=\'leave_review_rating\']').val('5');
                        }
                    }
                });
            }
    ...
    // leave-review-form
    if ( $('.modal-testimonial-form form').length ) {
        var form_item8 = $('.modal-testimonial-form form');
        form_item8.validate({
            rules: {
                leave_review_name: {
                    required: true
        ...
        form_item5.validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    ...
                    type: 'post',
                    dataType: 'json',
                    data: $("#form-quick-order").serialize(),
                    beforeSend: function() {
                    },
                    complete: function() {
                    },
    ...
    if ( $('.modal-callback-form form').length ) {
        ...
        form_item7.validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    url: 'index.php?route=module/feedback/send',
                    type: 'post',
                    dataType: 'json',
...
function sliderRange(){
    if ( $('#range-slider').length ){
        ...
        var max_value = parseInt($('#sidebar_price_to').val());
...
function countdownTimer(){
    if ( $('.js-clock').length ) {
        ...
        var clock = clock_block.FlipClock({
...
function sliders(){
    ...
    if ( $('.js-main-similar-products-slider').length && $('.mobile-res').is(':visible') ){
        main_similar_products_sliders_array = [];
        $('.js-main-similar-products-slider').each(function(i){
            var cur = $(this);
            cur.attr('data-main-similar-products-slider',i);
            main_similar_products_sliders_array[i] = cur.bxSlider({
                controls:false,
                adaptiveHeight:true,
                slideMargin: 20,
                oneToOneTouch:false,
...
function scrollpanes(){
    if ( !$('.mobile-res').is(':visible') ) {
        ...
        if ( $('.products-scrollbar').length ) {
            var product_scrollbar_jsp_array = [];
            $('.products-scrollbar').each(function(i){
                var cur = $(this);
                cur.attr('data-scrollbar',i);
                cur.find('.scrollpane-horizontal').jScrollPane({
                    verticalDragMaxHeight:0,
                    verticalDragMinHeight:0,
                    horizontalDragMinWidth: 120,
                    horizontalDragMaxWidth: 120,
                ...
                product_scrollbar_jsp_array[i] = cur.find('.scrollpane-horizontal').data('jsp');
            });

            // left
            $('.products-scrollbar__controls-left').on('click',function(e){
                var parent = $(this).parents('.products-scrollbar');
                var parent_index = parent.attr('data-scrollbar');
                var cur_product = parent.find('.product.current');
                if ( cur_product.prev().length ){
                    ...
                    product_scrollbar_jsp_array[parent_index].scrollToX(cur_product_prev_left);
                }
                e.preventDefault();
            });
            ...
            $('.products-scrollbar__controls-right').on('click',function(e){
                ...
                if ( cur_product.nextAll().length > 3 ){
                    ...
                    product_scrollbar_jsp_array[parent_index].scrollToX(cur_product_next_left);
                }
                e.preventDefault();
            });
        }
    ...
    if ( $('.js-print-link').length ) {
        $('.js-print-link').on('click',function(e){
            ...
            docprint.document.write('<meta charset="utf-8">\
                ...
                <link type="text/css" rel="stylesheet" href="design/css/materialize.min.css"  media="all"/>\
                <link type="text/css" rel="stylesheet" href="design/css/main.css"  media="all"/>\
                <link type="text/css" rel="stylesheet" href="design/css/no-print.css" media="print"/>\
                <link type="text/css" rel="stylesheet" href="design/css/print.css"  media="all"/></head><body>');

            // body
            docprint.document.write('<div class="no-print col s12 center-align">');
            docprint.document.write('  <div class="buyer-info-form__buttons center-align">');
            docprint.document.write('    <a onclick="print()" class="waves-effect waves-light btn-large btn-orange">Печать</a>');
...
/* mainTabs */
function mainTabs(){
    if ( $('.main-catalog .tabs').length ) {
        $('.main-catalog ul.tabs a').on('click',function(){
            if ( $('.mobile-res').is(':visible') ){
                setTimeout(function(){
                    var cur_slider_index = $('.main-catalog-tab:visible .js-main-similar-products-slider').attr('data-main-similar-products-slider');
                    main_similar_products_sliders_array[cur_slider_index].reloadSlider();
                },100);
            }
        });
    }
